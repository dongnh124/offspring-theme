...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
      ...
      if (variant) {
        ...
        for (let i = 1; i <= 3; i++) {
          ...
          if (variant[option] !== null){
            let valueOption = variant[option];
    ...
    switchProductImage: function(imageId) {
      ...
      //Scroll to active
      if(this.settings.sectionId === 'product-template-2' || this.settings.sectionId === 'product-template-7'){
      ...
      */
      _productThumbnailSlider: function() {
        ...
        if ($productThumbs.length > 1) {
          ...
          }else if(this.settings.sectionId === 'product-template-6'){
            $productThumbsWrapper.slick({
              accessibility: false,
              arrows: true,
              dots: true,
...
theme.Instagrams = (function() {
  function Instagrams(container) {
    ...
    this.settings = {
      ...
      styleItem       : this.$container.data('styleitem') || 'style1',
    ...
    // modify HTML on bottom-contents.liquid/x-instagram
    var $template = $(`#x-instagram-${this.settings.styleItem}`);
...
theme.slickCarousel = (function (){
  function Carousels(container) {
    ...
    this.settings.slidesToShow992  		= (this.settings.slidesToShow - 2) > itemMobile ? (this.settings.slidesToShow - 2) : itemMobile;
    this.settings.slidesToShow768  		= (this.settings.slidesToShow - 3) > itemMobile ? (this.settings.slidesToShow - 3) : itemMobile;
    ...
    this.settings.slidesToScroll992  	= (this.settings.slidesToScroll - 2) > itemMobile ? (this.settings.slidesToScroll - 2) : itemMobile;
    this.settings.slidesToScroll768  	= (this.settings.slidesToScroll - 3) > itemMobile ? (this.settings.slidesToScroll - 3) : itemMobile;
...
theme.CollectionsList = (function() {
  ...
  CollectionsList.prototype = _.assignIn({}, CollectionsList.prototype, {
    ...
    _collectionListFix: function() {
      let numberRows = this.$container.find('.grid').data('number-rows');
      let $featureCards = this.$container.find('.featured-card');
      ...
      for (let i = 0; i < numberRows; i++) {
        ...
        let $currentRow = $featureCards.filter(
          "[data-row-number='" + (i + 1) + "']"
          );
        let $cardHeaders = $currentRow.find('.featured-card__header');

        // Find the max heights for each row
        $currentRow.each(function() {
          let $cardTitle = $(this).find('.featured-card__title');
          let $cardAction = $(this).find('.featured-card__action');
          let $cardImageWrapper = $(this).find('.featured-card__image-wrapper');
          let headerHeight =
          $cardTitle.outerHeight() + $cardAction.outerHeight() + 65;
          let wrapperHeight = $cardImageWrapper.outerHeight();
...
theme.freeShipping = (function(){
  ...
  function generate(cart){
    ...
    if (priceCart >= minOrderValue){
      $percentClass.css('width','100%').removeClass('progress-bar-striped bg-warning').addClass('bg-success');
      ...
      $percentClass.css('width',`${percent}%`).addClass('progress-bar-striped bg-warning');
    ...
    if (wishlistObject.length > 0){
      for (var i = 0; i < wishlistObject.length; i++) {
        ...
        Shopify.getProduct(productHandle,function(product){
          ...
          htmlProduct += `<div class="js-wishlist-item col-lg-4 col-md-6 col-12 mb-4">`;
          htmlProduct += `  <div class="js-wishlist-itembox">`;
          htmlProduct += `    <div class="d-flex flex-row p-3">`;
          htmlProduct += `      <a class="d-inline-block mr-4 js-wishlist-image" href="${product.url}">`;
          htmlProduct += `        <img src="${product.featured_image}"/>`;
          htmlProduct += `      </a>`;
          htmlProduct += `      <div><a href="${product.url}" class="title-prouct d-block h4" >${product.title}</a>`;
    ...
    if (compareObject.length > 0){
      ...
      for (var i = 0; i < compareObject.length; i++) {
        ...
        Shopify.getProduct(productHandle,function(product){
          ...
          htmlProduct += `  <hr /><button class="btn js-remove-compare btn-theme" data-handle="${product.handle}">${theme.strings.compareRemove}</button>`;
          htmlProduct += `</div>`;
...
theme.countdown = (function(){
  function initCountdown(){
    $(".js-countdown").each(function() {
      ...
      htmlLayout = "<ul class='list--inline'><li><span class='countdown-time'>%%D%%</span><span class='countdown-text'>Days</span></li><li><span class='countdown-time'>%%H%%</span><span class='countdown-text'>Hours</span></li><li><span class='countdown-time'>%%M%%</span><span class='countdown-text'>Mins</span></li><li><span class='countdown-time'>%%S%%</span><span class='countdown-text'>Secs</span></li></ul>";
      if (style === 'style2'){
       htmlLayout = "<ul class='list--inline'><li><span class='countdown-time'>%%D%%</span><span class='countdown-text'>D</span></li><li><span class='countdown-time'>%%H%%</span><span class='countdown-text'>H</span></li><li><span class='countdown-time'>%%M%%</span><span class='countdown-text'>M</span></li><li><span class='countdown-time'>%%S%%</span><span class='countdown-text'>S</span></li></ul>";
     };
     $(this).lofCountDown({
      TargetDate:endTime,
...
theme.swatchCard = (function(){
  $(document).on('click','.js-swatch-card-item',function(){//click or mouseover
    ...
    let id = $(this).data('id');
    let gridItem = $(this).closest('.js-product-card');
...
theme.swatchCard2 = (function(){
  function initVariant(id){
    let productJson = JSON.parse($('.customJson-' + id).html());
    let $selectorForm = $('.customform-' + id);
    let $button = $('.js-customform-addtocart-' + id);
    let $buttontext = $button.find('span');
    let $wrapObject = $selectorForm.closest('.js-product-card');
    let options = {
    ...
    let variants = new slate.Variants(options);
    let AjaxCart = new window.AjaxCart($selectorForm);
    let _updateButton = function (evt){
      let variant = evt.variant;
    ...
    let _updateImage = function (evt){
      let variant = evt.variant;
      let $mainImage = $wrapObject.find('.product-card__image').find('img');
      if (variant !== undefined && variant.featured_image !== null){
        let variantImage = variant.featured_image;
        $mainImage.attr('srcset',variantImage.src);
      }
    }
    let _updatePrice = function (evt){
      let $price = $wrapObject.find('.product-card__price');
      let variant = evt.variant;
      if (variant !== undefined){
        let htmlComparePrice = variant.compare_at_price !== null ? `<s class="product-card__regular-price"><span class="money">${variant.compare_at_price}</span></s>` : '';
        let htmlPrice = `${htmlComparePrice}<span class="money">${variant.price}</span>`;
...
theme.isotope = (function(){
  let enableIsotope = $('.enable-isotope').length === 1 ? true : false,
  $grid = $('.js-grid'),
  ...
  function initIsotope(){
    if (enableIsotope){
      ...
      $(document).on('click',filter,function(){
        let filterValue = $(this).data('filter') === '*'? $(this).data('filter') : '.' + $(this).data('filter');
...
theme.crosssell = (function(){
  let crosssellClass = '.js-crosssell',
  ...
  function generateCrosssell(){
    for (let i = 0; i < arrayHandle.length; i++) {
      ...
      Shopify.getProduct(productHandle,function(product){
        ...
        let productPrice = theme.Currency.formatMoney(firstAvailableVariant.price, theme.moneyFormat),
        productImage = (firstAvailableVariant.featured_image === null || firstAvailableVariant.featured_image === undefined)? product.featured_image:firstAvailableVariant.featured_image.src,
  ...
  if (handles !== '') {
    $(document).on('click',crossImageClass,function(event){
      event.preventDefault();
      let parent = $(this).closest('.cross-item'),
    ...
    $(document).on('change',variantSelectClass,function(){
      let selectVariant = $(this).children(':selected'),
    ...
    $button.on('click',function(){
      if($(this).hasClass(buttonAcceptClass)){
        Shopify.addItem(productUpsellId,1,function(item){
          theme.miniCart.updateElements();
          theme.miniCart.generateCart();
          let htmlVariant = item.variant_title !== null ? `<i>(${item.variant_title})</i>` : '';
          let htmlAlert = `<div class="media mt-2 alert--cart"><a class="mr-3" href="/cart"><img class="lazyload" data-src="${item.image}"></a><div class="media-body align-self-center"><p class="m-0 font-weight-bold">${item.product_title} x ${item.quantity}</p>${htmlVariant}<div><div>`;
          theme.alert.new(theme.strings.addToCartSuccess,htmlAlert,3000,'notice');
        })
      }
...
Shopify.onError = function(t, r) {
  let e = eval("(" + t.responseText + ")");
  let mess = e.message ? e.message + "(" + e.status + "): " + e.description : "Error : " + Shopify.fullMessagesFromErrors(e).join("; ") + "."
  theme.alert.new('Alert',mess,3000,'warning');
}

// Megamenu width
theme.fixWidthMegamenu = (function(){
  let $megamenuClass = $('.site-nav__item-mega'),
  megaDropdownClass = '.meganav';
  $megamenuClass.each(function(){
    let widthMenu = $(this).data('width') || 0;
    if (widthMenu !== 0){
...
theme.productSuggest = (function(){
  let $productSuggestClass = $('.product-notification'),
  ...
  function SomeonePurchased() {
    ...
    function toggleSomething() {
      if($productSuggestClass.hasClass('active')){
        ...
        let arrayProducts = $('.data-product'),
...
theme.stickyHeader = (function(){
  const stickHeaderClass = '.site-header--sticky';
  const $bundleFrom = $('#js-bundle-form');
  if ($(stickHeaderClass).length !== 0 && $bundleFrom.length === 0){
    let sticky = $(stickHeaderClass).offset().top + $(stickHeaderClass).height();
    $(window).scroll(function() {
...
theme.anchorScroll = (function(){
  $(document).on('click', 'a[href^="#"]', function (event) {
    event.preventDefault();
    let disableAnchor = $(this).hasClass('disabled-anchor');
    if (!disableAnchor){
      $('html, body').animate({
...
theme.bundle = (function(){
  let $bundleForm = $('#js-bundle-form'),
      BUNDLE_PACKAGE_SIZE = $bundleForm.data('max-quantity') || 0,
      EXTRA_PACKAGE_SIZE = $bundleForm.data('extra-max-quantity') || 0,
      FIRST_PACKAGE_SIZE = 0,
      SECOND_PACKAGE_SIZE = 0,
      subscriptionFrequency = $bundleForm.data('frequency') || 1,
      subscriptionUnit = $bundleForm.data('unit') || 'month',
      bundleType = 'single',
      bundleProducts = [],
      bundleItems = [],
      bundleFreebies = [],
  ...
  const enableNextButtonOfStep = (index) => {
    const $nextButton = $(`#step-${index}`).find('.js-next-btn');
    $nextButton.prop('disabled', false);
  }

  const disableNextButtonOfStep = (index) => {
    const $nextButton = $(`#step-${index}`).find('.js-next-btn');
    $nextButton.prop('disabled', true);
  }

  const getQuantityOf = ($selector) => {
    return parseInt($selector.data('quantity'));
  }

  const setQuantityText = ($selector, value) => {
    $selector.data('quantity', value).attr('data-quantity',value).text(value);
  }

  const getTotalQuantityOf = ($selector) => {
    let quantity = 0;
    $selector.find('.js-step-quantity').each(function() {
      const itemQuantity = getQuantityOf($(this));
      quantity += itemQuantity;
    })
    return quantity;
  }

  const setTotalQuantityOfStep = (index) => {
    const $stepQuantityText = $(`#step-${index}`).find('.js-step-total-quantity');
    const totalQuantity = getTotalQuantityOf($(`#step-${index}`));
    $stepQuantityText.text(totalQuantity)
  }

  const setTotalQuantityOfProduct = (index) => {
    const $stepQuantityText = $('#step-2').find('.js-step-total-quantity');
    const totalQuantity = getTotalQuantityOf($(`#js-product-${index}`));
    $stepQuantityText.text(totalQuantity);
  }

  const processNavigation = () => {
    var $navListItem = $('.btn-step'),
        $stepContent = $('.js-step-wrap'),
        $allNextBtn = $('.nextBtn'),
        $allPrevBtn = $('.prevBtn');

    const activeNavItemsFromOneTo = (index) => {
      $navListItem.removeClass('active');
      for (i = 1; i <= index; i++){
        $(`.btn-step-${i}`).addClass('active');
      }
    }
    ...
    $navListItem.click(function (e) {
      ...
      if (!$item.hasClass('disabled')) {
        activeNavItemsFromOneTo(stepIndex);
        $stepContent.hide();
        $target.show();
      }
    });
    ...
    $allNextBtn.click(function(){
      var currentStep = $(this).closest(".js-step-wrap"),
          currentStepBtn = currentStep.attr("id"),
          nextStepWizard = $('div.setup-panel div a[href="#' + currentStepBtn + '"]').parent().next().children("a"),
          isValid = true;
    ...
    $allPrevBtn.click(function(){
      var currentStep = $(this).closest(".js-step-wrap"),
          currentStepBtn = currentStep.attr("id"),
          prevStepWizard = $('div.setup-panel div a[href="#' + currentStepBtn + '"]').parent().prev().children("a");
  ...
  const processFirstStep = () => {
    const $step1 = $('#step-1');
    const $stepItems = $step1.find('.js-step-item');
    const $stepQuantityText = $step1.find('.js-step-total-quantity');
    const $nextButton = $step1.find('.nextBtn');
    const STEP_PACKAGE_SIZE = BUNDLE_PACKAGE_SIZE;

    const enableAllItems = () => {
      $stepItems.removeClass('disabled');
    }

    const disableItemsWithNoQuantity = () => {
      const totalQuantity = getTotalQuantityOf($('#step-1'));
      if (bundleProducts.length === 2 || totalQuantity === STEP_PACKAGE_SIZE ){
        $stepItems.each(function(){
          const $quantityBox = $(this).find('.js-step-quantity');
          if ($quantityBox.data('quantity') === 0) {
    ...
    const resetForm = () => {
      ...
      setQuantityText($step1.find('.js-step-quantity'),0);
      setTotalQuantityOfStep(1);
      disableNextButtonOfStep(1);
      enableAllItems();
      bundleProducts = [];
    }

    const minusHandleStep1 = () => {
      var totalQuantity = getTotalQuantityOf($('#step-1'));
      if (totalQuantity === 0) {
        enableAllItems();
      }
    }

    const plusHandleStep1 = () => {
      var totalQuantity = getTotalQuantityOf($('#step-1'));
      if (totalQuantity === STEP_PACKAGE_SIZE) {
        enableNextButtonOfStep(1);
      }
    }

    const processBundleRadioSelected = () => {
      $('.js-select-bundle').on('click',function(){
        resetForm();

        const $inputBox = $(this).find('input'),
              productType = $inputBox.data('bundle') || '';

        bundleType = $inputBox.data('type');
        $step1.removeClass('single mix').addClass(bundleType);

        if (productType !== ''){
          $stepItems.parent().addClass('hide');
          $(`.product-type-${productType}`).parent().removeClass('hide');
        }else{
          $stepItems.parent().removeClass('hide');
        }

        $step1.find('.js-step-max-quantity').text(STEP_PACKAGE_SIZE);
      })

      //Initial
      $('.js-select-bundle').first().trigger('click');
    }

    const generateVariantFromHandle = (handle,$selector) => {
      jQuery.getJSON(`/products/${handle}.js`, function(product) {
        ...
        variants.map(variant => {
          const variantImageSrc = variant.featured_image !== null ? theme.Images.getSizedImageUrl(variant.featured_image.src,'600x') : '';
          const $template = $('#x-bundle-variant');
          const context = {
            productHandle		: productHandle,
            variantId           : variant.id,
            variantImage        : variantImageSrc,
            variantTitle        : variant.title
          }

          const templateScript = Handlebars.compile($template.html());
          const itemHtml = templateScript(context);
    ...
    const generateSecondStep = () => {
      const $step2 = $('#step-2');
      const $maxQuantityVariant = $step2.find('.js-step-max-quantity');
      const itemHandle1 = bundleProducts[0];

      // Reset form
      $step2.find('.js-step-total-quantity').html('0');
      disableNextButtonOfStep(2);
      bundleItems = [];


      generateVariantFromHandle(itemHandle1,$('#js-product-1'));
      FIRST_PACKAGE_SIZE = getQuantityOf($(`#js-product-quantity-${itemHandle1}`));
      $maxQuantityVariant.html(FIRST_PACKAGE_SIZE);

      const showFakeButton = (typeButton) => {
        $step2.find(`.${typeButton}Btn`).addClass('hide');
        $step2.find(`.${typeButton}BtnFake`).removeClass('hide');
      }

      const hideFakeButton = (typeButton) => {
        $step2.find(`.${typeButton}Btn`).removeClass('hide');
        $step2.find(`.${typeButton}BtnFake`).addClass('hide');
      }

      if (bundleProducts.length > 1) {
        const itemHandle2 = bundleProducts[1];
        SECOND_PACKAGE_SIZE = getQuantityOf($(`#js-product-quantity-${itemHandle2}`));
        generateVariantFromHandle(itemHandle2,$('#js-product-2'));
        showFakeButton('next');

        $('.nextBtnFake').on('click',function(){
          $('#js-product-1').addClass('hide');
          $('#js-product-2').removeClass('hide');
          showFakeButton('prev');
          hideFakeButton('next');
          $maxQuantityVariant.html(SECOND_PACKAGE_SIZE);
        })

        $('.prevBtnFake').on('click',function(){
          $('#js-product-1').removeClass('hide');
          $('#js-product-2').addClass('hide');
          hideFakeButton('prev');
          showFakeButton('next');
          $maxQuantityVariant.html(FIRST_PACKAGE_SIZE);
        })
    ...
    $step1.find('.js-step-minus').on('click',function(){
      const $quantityBox = $(this).parent().find('.js-step-quantity'),
            itemQuantity = getQuantityOf($quantityBox),
            itemHandle = $quantityBox.data('handle'),
            newItemQuantity = itemQuantity - 1;
      if (itemQuantity > 0){
        setQuantityText($quantityBox,newItemQuantity);
      }
      if (newItemQuantity === 0){
        bundleProducts.splice(bundleProducts.indexOf(itemHandle), 1);
      }

      disableNextButtonOfStep(1);
      disableItemsWithNoQuantity();
      setTotalQuantityOfStep(1);
      minusHandleStep1();
    })

    $step1.find('.js-step-plus').on('click',function(){
      const $currentProduct = $(this).closest('.js-step-item'),
            $quantityBox = $currentProduct.find('.js-step-quantity'),
            itemHandle = $quantityBox.data('handle'),
            itemQuantity = getQuantityOf($quantityBox);
      ...
      if (bundleType === 'single'){
        setQuantityText($step1.find('.js-step-quantity'),0);
        newItemQuantity = STEP_PACKAGE_SIZE;
        setQuantityText($quantityBox,newItemQuantity);
        ...
        bundleProducts = [];
        bundleProducts.push(itemHandle);
      }else{
        const totalQuantity = getTotalQuantityOf($('#step-1'));
        if (itemQuantity < STEP_PACKAGE_SIZE && totalQuantity < STEP_PACKAGE_SIZE){
          setQuantityText($quantityBox,newItemQuantity);
        }
        if (bundleProducts.indexOf(itemHandle) === -1){
          bundleProducts.push(itemHandle);
        }
        disableItemsWithNoQuantity();
      }
    ...
    processBundleRadioSelected ();
    $nextButton.on('click',function(){
      generateSecondStep();
    })
  }

  const processSecondStep = () => {
    const $step2 = $('#step-2');
    const $stepItems = $step2.find('.js-step-item');
    const $stepQuantityText = $step2.find('.js-step-total-quantity');
    const $nextButton = $step2.find('.nextBtn');

    const plusHandleStep2 = (productIndex) => {
      const PACKAGE_SIZE = productIndex === 1 ? FIRST_PACKAGE_SIZE : SECOND_PACKAGE_SIZE;
      const totalQuantity  = getTotalQuantityOf($(`#js-product-${productIndex}`));
      if (totalQuantity === PACKAGE_SIZE) {
        enableNextButtonOfStep(2);
      }
    }

    const minusHandleStep2 = (productIndex) => {
      const PACKAGE_SIZE = productIndex === 1 ? FIRST_PACKAGE_SIZE : SECOND_PACKAGE_SIZE;
      const totalQuantity  = getTotalQuantityOf($(`#js-product-${productIndex}`));
      if (totalQuantity < PACKAGE_SIZE) {
        disableNextButtonOfStep(2);
    ...
    $(document).on('click','#step-2 .js-step-minus', function() {
      const $quantityBox = $(this).parent().find('.js-step-quantity'),
            itemQuantity = getQuantityOf($quantityBox),
            itemHandle = $quantityBox.data('handle'),
            newItemQuantity = itemQuantity - 1;
      const productIndex = $(this).closest('.js-bundle-product').data('prd');

      if (itemQuantity > 0){
        setQuantityText($quantityBox,newItemQuantity);
      }
      if (newItemQuantity === 0){
        bundleItems.splice(bundleItems.indexOf(itemHandle), 1);
      }

      minusHandleStep2(productIndex);
      setTotalQuantityOfProduct(productIndex);
    })

    $(document).on('click','#step-2 .js-step-plus', function() {
      const $currentProduct = $(this).closest('.js-step-item'),
            $quantityBox = $currentProduct.find('.js-step-quantity'),
            itemHandle = $quantityBox.data('handle'),
            itemQuantity = getQuantityOf($quantityBox);
      ...
      const totalQuantity = getTotalQuantityOf($(`#js-product-${productIndex}`));
      if (itemQuantity < PACKAGE_SIZE && totalQuantity < PACKAGE_SIZE){
        setQuantityText($quantityBox,newItemQuantity);
        if (bundleItems.indexOf(itemHandle) === -1){
          bundleItems.push(itemHandle);
        }
      }

      plusHandleStep2(productIndex);
      setTotalQuantityOfProduct(productIndex);
    })

    $('.prevBtnFake').on('click',function(){
      setTotalQuantityOfProduct(1);
      enableNextButtonOfStep(2);
    })

    $('.nextBtnFake').on('click',function(){
      setTotalQuantityOfProduct(2);
      const totalQuantityProduct2  = getTotalQuantityOf($('#js-product-2'));
      if (totalQuantityProduct2 !== SECOND_PACKAGE_SIZE) {
        disableNextButtonOfStep(2);
      }
    })
  ...
  const processThirdStep = () => {
    const $step3 = $('#step-3');
    const $stepItems = $step3.find('.js-step-item');
    const $nextButton = $step3.find('.nextBtn');

    $step3.find('.js-step-plus').on('click', function(){
      const $freebieItem = $(this).closest('.js-step-item');
      const freebieId = $(this).data('first-variant');
      $stepItems.removeClass('active');

      if (bundleFreebies.indexOf(freebieId) === -1){
        bundleFreebies= [];
        bundleFreebies.push(freebieId);
        $freebieItem.addClass('active');
        enableNextButtonOfStep(3);
      }else{
        bundleFreebies= [];
        disableNextButtonOfStep(3);
      }
    })
  }
  ...
  const processFourthStep = () => {
    const $step4 = $('#step-4');
    const $stepItems = $step4.find('.js-step-item');
    const $nextButton = $step4.find('.nextBtn');

    $step4.find('.js-step-plus').on('click', function(){
      const $currentProduct = $(this).closest('.js-step-item');
      const extraId = $currentProduct.data('first-variant');
      const $quantityBox = $currentProduct.find('.js-step-quantity');
      const itemQuantity = getQuantityOf($quantityBox);
      const totalQuantity = getTotalQuantityOf($step4);
      if (itemQuantity < EXTRA_PACKAGE_SIZE && totalQuantity < EXTRA_PACKAGE_SIZE){
        setQuantityText($quantityBox,itemQuantity + 1);
        if (bundleExtras.indexOf(extraId) === -1){
          bundleExtras.push(extraId);
        }
      }

      setTotalQuantityOfStep(4)
    })

    $step4.find('.js-step-minus').on('click', function(){
      const $currentProduct = $(this).closest('.js-step-item');
      const extraId = $currentProduct.data('first-variant');
      const $quantityBox = $currentProduct.find('.js-step-quantity');
      const itemQuantity = getQuantityOf($quantityBox);
      ...
      if (newItemQuantity === 0){
        bundleExtras.splice(bundleExtras.indexOf(extraId), 1);
      }
    ...
    $nextButton.on('click',function(){
      ...
      const BUNDLES = bundleItems.map(bundleItem => {
        const productHandle = $(`#js-variant-quantity-${bundleItem}`).closest('.js-step-item').data('product');
        const quantity = $(`#js-variant-quantity-${bundleItem}`).data('quantity');
        return {
          product:productHandle,
          variantId:bundleItem,
          quantity:quantity,
          subscription:true
        };
      })

      const FREEBIES = bundleFreebies.map(freebie => {
        const productHandle = $(`.freebie-${freebie}`).data('product');
        const mapping = $(`.freebie-${freebie}`).data('mapping');
        return {
          product:productHandle,
          variantId:freebie,
          quantity:1,
      ...
      const EXTRAS = bundleExtras.map(extra => {
        const productHandle = $(`.extra-${extra}`).data('product');
        const mapping = $(`.extra-${extra}`).data('mapping');
        const quantity = $(`.extra-${extra}`).find('.js-step-quantity').data('quantity');
        return {
          product:productHandle,
          variantId:extra,
          quantity:quantity,
      ...
      const TOTALS = BUNDLES.concat(FREEBIES).concat(EXTRAS);

      const generateConfirmItem = ($selector,objects) => {
        $selector.html('');
        if (objects.length > 0){
          objects.forEach(object => {
            const productHandle = object.product,
                  variantId = object.variantId,
                  quantity = object.quantity;
            jQuery.getJSON(`/products/${productHandle}.js`, function(product) {
              const productTitle = product.title;
              const productImageSrc = product.featured_image !== null ? theme.Images.getSizedImageUrl(product.featured_image,'40x') : '';
              const variants = product.variants;

              variants.map(variant => {
                if (variant.id === variantId) {
                  const variantPrice = (object.mapping !== null && object.mapping !== undefined) ? parseFloat(object.mapping.price) * 100 : variant.price;
                  const variantComparePrice = (object.mapping !== null && object.mapping !== undefined) ? variant.price : '';
                  const variantImageSrc = variant.featured_image !== null ? theme.Images.getSizedImageUrl(variant.featured_image.src,'40x') : '';
                  const itemImageSrc = product.variants.length > 1 ? variantImageSrc : productImageSrc;
                  const $template = $('#x-review-item');
                  const variantTitle = variants.length > 1 ? `${variant.title} - ` : '';
                  const context = {
      ...
      const calculateTotal = ($selector,objects) => {
        var totalPrice = 0;
        objects.forEach(object => {
          const productHandle = object.product,
                variantId = object.variantId,
          ...
          jQuery.getJSON(`/products/${productHandle}.js`, function(product) {
            ...
            variants.map(variant => {
              if (variant.id === variantId) {
                const variantPrice = (object.mapping !== null && object.mapping !== undefined) ? parseFloat(object.mapping.price) * 100 : variant.price;
                const itemPrice = variantPrice * quantity;
                totalPrice += itemPrice;
                const htmlTotalPrice = theme.Currency.formatMoney(totalPrice, theme.moneyFormat);
                $selector.html(htmlTotalPrice);
              }
            })
          })
        })
      ...
      const processAddToCart = (objects) => {
        const $addToCartButton = $('.js-bundle-add-to-cart');
        $addToCartButton.on('click',function(){
          let formData = {
            'items':[]
          };
          objects.forEach(object => {
            const itemId = (object.mapping !== null && object.mapping !== undefined) ? object.mapping.variant : object.variantId;
            const subscriptions = object.subscription ? {
      ...
      generateConfirmItem($('.js-confirm-bundle'),BUNDLES);
      generateConfirmItem($('.js-confirm-freebie'),FREEBIES);
      generateConfirmItem($('.js-confirm-extra'),EXTRAS);

      calculateTotal($('.js-total-price'),TOTALS);
      processAddToCart(TOTALS);
...
theme.productRecentlyViewed = (function(){
  let $viewedElement = $('.js-recently-view'),
  ...
  function generateProduct(){
    ...
    if (arrayProduct.length > 0){
      ...
      for (let i = 0; i < arrayProduct.length; i++) {
        let productHandle = arrayProduct[i];
        Shopify.getProduct(productHandle,function(product){
          let $template = $('#x-product-grid');
          let context = {
            priceVaries           : product.price_varies,
            image                 : product.featured_image,
            productUrl            : product.url,
            productName           : product.title,
            productPrice          : product.price_varies ? product.price_min : product.price,
            productComparePrice   : product.compare_at_price
          }

          // modify HTML on bottom-contents.liquid/x-product-grid
          let templateScript = Handlebars.compile($template.html());
          let itemHtml = templateScript(context);
...
theme.noticeSoldout = (function(){
  let $soldoutWrapFormClass = $('.js-contact-soldout'),
  $btnWaitlist = $('.js-btn-waitlist'),
...
theme.stickyCart = (function(){
  let $anchor = $('#js-anchor-sticky-cart'),
...
theme.tooltip = (function(){
  let selector = '[data-toggle="tooltip"],[data-tooltip="true"]';
...
theme.sidebar = (function(){
  let togglebutton = '.js-toggle-filter',
  $sidebar = $('.js-sidebar');
...
theme.ajaxFilter = (function() {
  ...
  var apollo = {
    ...
    filterMapData: function(data) {
      ...
      // 4. update count
      $(ajaxCount).html($(ajaxCount, data.responseText).html());
    },
